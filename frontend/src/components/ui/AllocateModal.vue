...
<!--            </option>-->
<!--          </select>-->
<!--          <p v-if="mode==='HQ' && selectedCenter && !selectedCenter.hasManager"-->
<!--             class="text-xs text-amber-600 mt-1">-->
<!--            이 센터는 센터장이 없습니다. 센터 소속 직원을 선택해야 분배할 수 있어요.-->
<!--          </p>-->
<!--        </div>-->

<!--        &lt;!&ndash; 직원 검색 (선택) &ndash;&gt;-->
<!--        <div class="space-y-2">-->
...
<!--            :disabled="submitting || (mode==='HQ' && (!centerId || (!selectedCenter?.hasManager && !pickedUser)))"-->
<!--            @click="confirm"-->
<!--        >-->
...
<!--    centers.value = data || []-->
<!--    // if (!centerId.value && centers.value.length) centerId.value = centers.value[0].centerId-->
...
<template>
  <div class="fixed inset-0 z-[100000] flex items-center justify-center bg-black/40 p-4"
    ...
    <div class="w-full max-w-[700px] rounded-2xl bg-white dark:bg-gray-900 shadow-xl border border-gray-200 dark:border-gray-800">
        ...
        <div class="space-y-2">
          ...
          <div class="flex gap-2">
            <input
                ...
                class="flex-1 h-11 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm dark:bg-gray-800 dark:text-gray-200 disabled:opacity-60"/>
            <button
                @click="searchUsers"
                :disabled="submitting || (mode==='HQ' && !centerId)"
                class="h-11 px-4 rounded-lg border border-gray-200 dark:border-gray-700 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-60">
              검색
            </button>
          </div>
      ...
      <div class="px-5 py-3 border-t border-gray-200 dark:border-gray-800 flex items-center justify-end gap-2">
        <button
            class="h-10 px-4 rounded-lg border border-gray-200 dark:border-gray-700 text-sm hover:bg-gray-50 dark:hover:bg-gray-800"
            @click="$emit('close')">
          취소
        </button>
        <button
